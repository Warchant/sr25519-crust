...
pub unsafe extern "C" fn ext_sr_from_seed(seed_ptr: *const u8) -> *mut u8 {
	...
	create_from_seed(seed).to_bytes().to_vec().as_mut_ptr()
}
...
pub unsafe extern "C" fn ext_sr_sign(
	...
	let public = slice::from_raw_parts(public_ptr, 32);
	let secret = slice::from_raw_parts(secret_ptr, 64);
...
pub mod tests {
	...
	fn can_create_keypair() {
		...
		let keypair = unsafe { ext_sr_from_seed(seed.as_ptr()) };
	...
	fn creates_pair_from_known() {
		...
		let keypair = unsafe { slice::from_raw_parts(keypair_ptr, 64) };
	...
	fn soft_derives_pair() {
		...
		let derived = unsafe { slice::from_raw_parts(derived_ptr, 32) };
